    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">{filtered.length} turnos encontrados</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={cn(
              "px-3 py-2 rounded-xl font-bold text-sm flex items-center gap-2 transition-all",
          ...
          <button
            onClick={exportCSV}
            className="px-3 py-2 bg-white/5 border border-white/10 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-white/10 transition-all"
          >
      ...
      {/* Filters */}
      {showFilters && (
        <div className="p-4 bg-white/5 border border-white/5 rounded-2xl space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 space-y-1">
              ...
              <input
                ...
                className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-sm focus:border-gold-500"
              />
            </div>
            <div className="flex-1 space-y-1">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-500">Total: <span className="text-gold-500 font-bold">{formatCurrency(totalRevenue)}</span></span>
            {(searchTerm || dateFrom !== format(subDays(new Date(), 7), "yyyy-MM-dd") || dateTo !== format(addDays(new Date(), 30), "yyyy-MM-dd")) && (
              <button
